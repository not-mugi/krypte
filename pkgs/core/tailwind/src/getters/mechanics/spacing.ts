import type {
  MeasurementScales, SpacingMeasurements, AxisSpacingMeasurements,
...
export const getSpacingClass = (spacing?: SpacingMeasurements) => {
  ...
  if (spacing && spacing in NEGATIVE_FIXED_SPACING_MAP) {
    return NEGATIVE_FIXED_SPACING_MAP[
      spacing as AxisSpacingNegativeMeasurements
    ];
  }
};

/** @alpha */
export const getMarginClass = (margin?: MarginMeasurements) => {
  ...
  if (margin && margin in NEGATIVE_FIXED_SIDES_MARGIN_MAP) {
    return NEGATIVE_FIXED_SIDES_MARGIN_MAP[
      margin as NegativeSideMarginMeasurements
    ];
  }
  if (margin && margin in NEGATIVE_FIXED_AXIS_MARGIN_MAP) {
    return NEGATIVE_FIXED_AXIS_MARGIN_MAP[
      margin as NegativeAxisMarginMeasurements
    ];
  }
  if (margin && margin in NEGATIVE_FIXED_LOGICAL_MARGIN_MAP) {
    return NEGATIVE_FIXED_LOGICAL_MARGIN_MAP[
      margin as NegativeLogicalMarginMeasurements
    ];
  }
};

/** @alpha */
export const getPaddingClass = (padding?: PaddingMeasurements) => {
